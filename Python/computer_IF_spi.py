...
class Device_uart:
    def __init__(self, dev, speed=921600):
        self.serial = serial.Serial(dev, speed)
        self.serial.write(bytes([OPCODE_READ_IFVER]))
        self.IFver = Resp_msg(self, ACK_Single).data
        self.serial.write(bytes([OPCODE_READ_PROJID]))
        self.ProjID = Resp_msg(self, ACK_Single).data

    def _write_single(self, data, addr):
        self.serial.write(bytes([OPCODE_WRITE_SINGLE, (addr//256)%256, addr%256, (data//256)%256, data%256]))
        return Resp_msg(self, ACK_Short)
        
    def _read_single(self, addr):
        self.serial.write(bytes([OPCODE_READ_SINGLE, (addr//256)%256, addr%256]))
        return Resp_msg(self, ACK_Single)

    def _read_map(self, addr, length):
        self.serial.write(bytes([OPCODE_READ_MAP, (addr//256)%256, addr%256, (length//256)%256, length%256]))
        r = Resp_msg(self, ACK_Stream_Start)
        if r.ok:
            dat = self.serial.read(length*2)
    ...
    def _read_FIFO(self, addr, length):
        self.serial.write(bytes([OPCODE_READ_FIFO, (addr//256)%256, addr%256, (length//256)%256, length%256]))
        r = Resp_msg(self, ACK_Stream_Start)
        if r.ok:
            dat = self.serial.read(length*2)
            resp_data = [dat[i]*256+dat[i+1] for i in range(0, length*2, 2)]
    ...
    def _write_map(self, addr, data):
        length = len(data)
        self.serial.write(bytes([OPCODE_WRITE_MAP, (addr//256)%256, addr%256, (length//256)%256, length%256, (data[0]//256)%256, data[0]%256]))
        r = Resp_msg(self, ACK_Short)
        if r.ok:
            dat = [i.to_bytes(2, byteorder='big') for i in data[1:]]
            self.serial.write(b''.join(dat))
            r.end_of_stream()
        return r

    def _write_FIFO(self, addr, data):
        length = len(data)
        self.serial.write(bytes([OPCODE_WRITE_FIFO, (addr//256)%256, addr%256, (length//256)%256, length%256, (data[0]//256)%256, data[0]%256]))
        r = Resp_msg(self, ACK_Short)
        if r.ok:
            dat = [i.to_bytes(2, byteorder='big') for i in data[1:]]
            self.serial.write(b''.join(dat))
    ...
    def _nop(self):
        self.serial.write(bytes([OPCODE_NOP]))
        return Resp_msg(self, ACK_Short)
...
class Device_spi:
    def __init__(self, dev, speed=921600):
        ...
        self.spi.open(0,0)
        self.spi.maximum_speed_hz(speed)
        self.spi.writebytes([OPCODE_READ_IFVER])
        self.IFver = Resp_msg(self, ACK_Single).data
        self.spi.writebytes([OPCODE_READ_PROJID])
        self.ProjID = Resp_msg(self, ACK_Single).data

    def _write_single(self, data, addr):
        self.spi.writebytes([OPCODE_WRITE_SINGLE, (addr//256)%256, addr%256, (data//256)%256, data%256])
        return Resp_msg(self, ACK_Short)
        
    def _read_single(self, addr):
        self.spi.writebytes([OPCODE_READ_SINGLE, (addr//256)%256, addr%256])
        return Resp_msg(self, ACK_Single)

    def _read_map(self, addr, length):
        self.spi.writebytes([OPCODE_READ_MAP, (addr//256)%256, addr%256, (length//256)%256, length%256])
        r=Resp_msg(self,ACK_Stream_Start)
        if r.ok:
            dat = self.spi.readbytes(length*2)
            resp_data = [dat[i]*256+dat[i+1] for i in range(0, length*2, 2)]
            r.end_of_stream()
            resp_data = resp_data[:r.dataok]
            return resp_data
    ...
    def _read_FIFO(self, addr, length):
        self.spi.writebytes([OPCODE_READ_FIFO, (addr//256)%256, addr%256, (length//256)%256, length%256]) 
        r = Resp_msg(self, ACK_Stream_Start)
        if r.ok:
            dat = self.spi.readbytes(length*2)
            resp_data = [dat[i]*256+dat[i+1] for i in range(0, length*2, 2)]
    ...
    def _write_map(self, addr, data): 
        length = len(data)
        self.spi.writebytes([OPCODE_WRITE_MAP, (addr//256)%256, addr%256, (length//256)%256, length%256, (data[0]//256)%256, data[0]%256])
        r = Resp_msg(self, ACK_Short)
        if r.ok:
            dat = [i.to_bytes(2, byteorder='big') for i in data[1:]]
            self.spi.writebytes([b''.join(dat)])
            r.end_of_stream()
        return r

    def _write_FIFO(self, addr, data):
        length = len(data)
        self.spi.writebytes([OPCODE_WRITE_FIFO, (addr//256)%256, addr%256, (length//256)%256, length%256, (data[0]//256)%256, data[0]%256])
        r = Resp_msg(self, ACK_Short)
        if r.ok:
            dat = [i.to_bytes(2, byteorder='big') for i in data[1:]]
            self.spi.writebytes([b''.join(dat)])
    ...
    def _nop(self):
        self.spi.writebytes([OPCODE_NOP])
        return Resp_msg(self, ACK_Short)
